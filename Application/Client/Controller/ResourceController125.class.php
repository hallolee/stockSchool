...
END:
    ...
    public function edit()
    {
        ...
        if (is_numeric($raw['id'])) {
            ...
            if(empty($data_ch)){
                $res_check1 = D('Client/Check')->selectCheckWithExt(
                    '',
                    [
                        'b.pid'     =>$exist['check_id'],
                        'a.modify'  =>MODIFY
                    ]
                );
                if($res_check1)
...
END:
    ...
    public function reply()
    {
        ...
        $file_exist = $this->model->findFile('id,title,ctime', ['id' => $raw['file_id'], 'check' => PASS]);
        if (!$file_exist) {
            $ret['status'] = E_NOEXIST;
            goto END;
        }
        ...
        $file_exist = $this->model->incFile(['field'=>'reply_n','value'=>1],['id'=>$raw['file_id']]);
...
END:
    ...
    public function myFiles()
    {
        ...
        $column = 'a.id,a.uid,a.title,a.img_path,a.browse_n,a.reply_n,a.atime,a.mtime,a.check,a.rem,
        b.data';
...
END:
    ...
    public function lists()
    {
        ...
        $result = $this->model->selectFile('id,img_path,title,browse_n,atime', $where, $limit, 'top asc,atime desc');
...
END:
    ...
    public function detail()
    {
        ...
        $head = C('PREURL');
        $res['img_url'] = $res['img_path'] ? $head . $res['img_path'] : '';
        ...
        $res['icon'] = $res['icon']?$head.$res['icon']:'';
        $ret = $res;
