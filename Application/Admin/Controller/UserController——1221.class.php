...
class UserController extends GlobalController
{
    ...
    public function userList()
    {
        ...
        if($raw['up_xiaozhu'])
            $where['d.uid'] = $raw['up_xiaozhu'];
        if($raw['uid'])
            $where['a.uid'] = $raw['uid'];
        if($raw['xiaozhu'])
            $where['a.uid'] = $raw['xiaozhu'];
...
END:
    ...
    public function editUser()
    {
        ...
        $uid = $raw['uid'];
        $keys = ['nickname','user','pass','icon','level','name','phone','work_age','city','qq','roles','email','teacher_uid','department'];
        foreach ($keys as $v){
            if($raw[$v])
                $data[$v] = $raw[$v];
        }
        ...
        if( isset( $data['level'] ) ) $data['level_id'] = $data['level'];
