...
class HomeModel extends GlobalModel
{
    ...
    public function selectNavWithItems($column='',$where='',$limit='',$sort='')
    {
        $info =  M(TNAV)
            ->alias('a')
            ->field($column)
            ->join('LEFT JOIN '.TSYSTEM_INFO_TYPE.' b on b.id = a.item_id')
            ->where($where)
            ->limit($limit)
            ->order($sort)
            ->select();
    ...
    public function findNavTop($column='',$where='')
    {
        $res =  M(TNAV)
            ->field($column)
            ->where($where)
    ...
    public function selectNavItem($column='',$where='',$limit='',$order='')
    {
        $res =  M(TNAV)
            ->alias('a')
            ->field($column)
            ->join('LEFT JOIN '.TSYSTEM_INFO_TYPE.' c on a.item_id =c.id')
            ->where($where)
            ->limit($limit)
            ->order($order)
            ->select();
