...
class MembershipModel extends GlobalModel
{
    ...
    public function selectRecordUpgrade($column = '', $where = '', $limit = '', $order = '')
    {
        $res = M( TUSER_UPGRADE_REC )
            ->alias('a')
            ->field($column)
            ->join('INNER JOIN '.TCHECK.' b on b.id = a.check_id')
            ->where($where)
            ->limit($limit)
            ->order($order)
            ->select();
    ...
    public function addRecordChgTeaAll($data)
    {
        $re = M( TUSER_CHGTEA_REC )
            ->addAll( $data );
    ...
    public function findRecordChgTea($column = '', $where = '')
    {
        $res = M( TUSER_CHGTEA_REC )
            ->field($column)
            ->where($where)
    ...
    public function selectRecordChgStatus($column = '', $where = '', $limit = '', $order = '')
    {
        $res = M(TUSER_CHG_STATUS_REC)
            ->alias('a')
            ->field($column)
            ->join('LEFT JOIN '.TCHECK.' b on b.id =a.check_id')
            ->where($where)
            ->limit($limit)
            ->order($order)
            ->select();
