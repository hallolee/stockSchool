...
class ResourceModel extends GlobalModel
{
    ...
    public function selectFileWithCheck($column = '',$where = '',$limit = '',$order = ''){

        $res = M(TFILE)
            ->alias('a')
            ->field($column)
            ->join('LEFT JOIN '.TCHECK_E.' b on b.status = 1 and b.tid = a.id')
            ->join('INNER JOIN '.TCHECK.' c on c.id = a.check_id')
            ->where($where)
            ->limit($limit)
            ->order($order)
            ->select();
    ...
    public function findFileWithUser($column = '',$where = '',$order = ''){
    ...
    public function findFileWithUid($column = '',$where = '',$limit = '',$order = ''){
